// AUTOR: Eduardo González Gutiérrez
// FECHA: 24 de abril de 2023
// EMAIL: alu0101461588@ull.edu.es
// VERSION: 2.0
// ASIGNATURA: Algoritmos y Estructuras de Datos
// PRÁCTICA Nº: 5
// ESTILO: Google C++ Style Guide
// COMENTARIOS: Clase TAD cola implementada con una lista

#ifndef QUEUE_H_
#define QUEUE_H_

#include <iostream>
#include <cassert>

#include "dll_t.h"

// Clase TAD cola implementada con una lista
template <class T> class queue_l_t {
 private:
  dll_t<T>    l_;

 public:
  // constructor
  queue_l_t(void) : l_() {}

  // destructor
  ~queue_l_t(void) {}

  // operaciones
  bool empty(void) const;
  int size(void) const;
  void push(const T& dato);
  void pop(void);
  const T& front(void) const;
  const T& back(void) const;

  // E/S
  std::ostream& write(std::ostream& os = std::cout) const;
};


// operaciones
/**
 * @brief Metodo que comprueba si una cola esta vacia
 * 
 * @tparam T 
 * @return true 
 * @return false 
 */
template<class T> bool queue_l_t<T>::empty(void) const {
  return l_.empty();
}

/**
 * @brief Metodo que devuelve el tamaño de una cola
 * 
 * @tparam T 
 * @return int 
 */
template<class T> int queue_l_t<T>::size(void) const {
  return l_.get_size();	
}

/**
 * @brief Metodo que inserta un elemento al comienzo de la cola
 * 
 * @tparam T 
 * @param dato 
 */
template<class T> void queue_l_t<T>::push(const T& dato) {
  dll_node_t<T>* node = new dll_node_t<T>(dato);
  assert(node != NULL);
  l_.push_front(node);
}

/**
 * @brief Metodo que extrae un elemento del final de la cola
 * 
 * @tparam T 
 */
template<class T> void queue_l_t<T>::pop(void) {
  assert(!empty());
  delete l_.pop_back();
}

/**
 * @brief Metodo que obtiene el primer elemento de la cola
 * 
 * @tparam T 
 * @return const T& 
 */
template<class T> const T& queue_l_t<T>::front(void) const {
  assert(!empty());
  return (l_.get_tail()->get_data());
}

/**
 * @brief Metodo que obtiene el ultimo elemento de la cola
 * 
 * @tparam T 
 * @return const T& 
 */
template<class T> const T& queue_l_t<T>::back(void) const {
  assert(!empty());
  return (l_.get_head()->get_data());
}

// E/S
/**
 * @brief Metodo para la escritura por pantalla
 * 
 * @tparam T 
 * @param os 
 * @return std::ostream& 
 */
template<class T> std::ostream& queue_l_t<T>::write(std::ostream& os) const {
  dll_node_t<T>* aux = l_.get_head();
  while (aux != NULL) {
    os << aux->get_data() << " ";
    aux = aux->get_next();
  }
  os << std::endl;
  return os;
}

/**
 * @brief Sobrecarga operador extraccion
 * 
 * @tparam T 
 * @param os 
 * @param q 
 * @return std::ostream& 
 */
template<class T> std::ostream& operator<<(std::ostream& os, const queue_l_t<T>& q) {
  q.write(os);
  return os;
}


#endif  // QUEUE_H_
